...
for epoch in range(trainning_epochs):
    ...
    with torch.no_grad():
        ...
        # test with TrainSet
        weight_loss = torch.FloatTensor(32, 784).to(device)
        weight_loss.fill_(0.00)
        ...
        for i, (test_x, test_y) in enumerate(MnistTrainSetloader):
            ...
            # print(i)
            # print('begin')
            # print(list(bn_model.parameters())) 
            # print('end')

            weight_loss += list(bn_model.parameters())[0]           # weight loss
            # print(weight_loss)
        ...
        weight_loss = weight_loss / train_total_batch               # weight loss
        weight_loss_np = weight_loss.cpu().numpy()
        # print('average = {}'.format(weight_loss_np))
        plt.hist(weight_loss_np, bins=100, density = True, alpha = 0.7, histtype='stepfilled')
        plt.show()
        ...
        train_losses.append([bn_loss.item(), origin_loss.item()])
        train_accs.append([bn_acc.item(), origin_acc.item()])
        ...
        valid_losses.append([bn_loss.item(), origin_loss.item()])
        valid_accs.append([bn_acc.item(), origin_acc.item()])
...
plot_compare(valid_accs, title = 'Validation A at Epoch')